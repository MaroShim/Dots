...
### Changed
- **Blue & Green 컬러 식별성 대폭 개선**:
  - 파랑과 초록이 작은 화면이나 야간 모드(블루라이트 차단 필터)에서 서로 구별하기 어렵다는 피드백 반영.
  - 기존 구글 블루(`#4285F4`)를 한층 더 밝고 청명한 **비비드 스카이 블루(`#00A0FF`)**로 변경.
  - 기존 초록(`#34A853`)을 짙고 차분한 **포레스트 그린(`#2E7D32`)**으로 변경.
  - 두 색 간의 명도(밝기) 차이를 $\Delta L^* = 17.4$로 극대화하여 저조도 환경이나 색각 이상 환경에서도 오인 없이 즉각 식별 가능하도록 개선.
  - PWA 및 터치 앱 아이콘 재성성 반영.
